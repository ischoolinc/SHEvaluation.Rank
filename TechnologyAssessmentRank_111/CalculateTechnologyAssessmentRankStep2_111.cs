...
(
    SELECT * FROM subject_score_data WHERE ns1 = '否' AND ns2 ='否' ---AND subject_required='必修' AND subject_requiredby='部訂'
)
"; // 過濾不需評分與不計學分 
...
";
                ...
                // 多學期技能領域部定必修加權平均--Cynthia(只有部定必修)
                StringBuilder sbSubjectSkillDomainReqScore = new StringBuilder();
                foreach (GradeYearSemesterInfo gs in calSemesterList)
                {
                    string ka = "";
                    if (gs.Semester == 1)
                    ...
                    // 解析技能領域
                    if (SubjectNameMapDict.ContainsKey("技能領域"))
                    {
                        if (SubjectNameMapDict["技能領域"].ContainsKey(ka))
                            inString = SubjectNameMapDict["技能領域"][ka];
                    }

                    if (inString != "")
                    {
                        string str = @", subject_score_SkillDomainReq_avg_" + ka + @" AS
(
    SELECT
        student_list.student_id
        ,student_list.student_name
    ...
    INNER JOIN student_list
        ON subject_score.student_id = student_list.student_id
    WHERE 
        subject_score.grade_year = " + gs.GradeYear + @" AND subject_score.semester = " + gs.Semester + @"  AND subject_required ='必修' AND subject_requiredby = '部訂' AND subject IN(" + inString + @")
    GROUP BY 
    student_list.student_id
        ,student_list.student_name
        ,student_list.rank_grade_year
...
";
                        sbSubjectSkillDomainReqScore.AppendLine(str);

                    }
                }

                string Strsubject_score_SkillDomainReq_avg_list = @"
,subject_score_SkillDomainReq_avg_list AS
(
SELECT
        student_id
        ,student_name
...
SELECT
        ...
        ,sum_credit
        FROM 
        subject_score_SkillDomainReq_avg_1a
UNION All 
SELECT
        student_id
        ,student_name
        ...
        ,sum_credit
        FROM 
        subject_score_SkillDomainReq_avg_1b
UNION All 
SELECT
        student_id
        ,student_name
        ...
        ,sum_credit
        FROM 
        subject_score_SkillDomainReq_avg_2a
UNION All 
SELECT
        student_id
        ,student_name
        ...
        ,sum_credit
        FROM 
        subject_score_SkillDomainReq_avg_2b
UNION All 
SELECT
        student_id
        ,student_name
        ...
        ,sum_credit
        FROM 
        subject_score_SkillDomainReq_avg_3a
            ) as subject_score_SkillDomainReq_avg5
group BY
student_id
        ,student_name
        ,rank_grade_year
        ...
        ,rank_tag2
        ,rank_group_name
    ) AS subject_score_SkillDomainReq_avg_round
)
,subject_score_SkillDomainReq_rank_list AS
(
    SELECT
         student_id
        ,student_name
        ...
        , COUNT(student_id) OVER(PARTITION BY rank_grade_year, rank_group_name) AS group_count
    FROM
        subject_score_SkillDomainReq_avg_list
    WHERE       
        avg_score IS NOT NULL
)
, subject_score_SkillDomainReq_avg_expand AS
(
    SELECT
        subject_score_SkillDomainReq_rank_list.*
        , FLOOR((grade_rank::DECIMAL - 1)*100::DECIMAL / grade_count) + 1 AS graderank_percentage
        , FLOOR((dept_rank::DECIMAL - 1)*100::DECIMAL / dept_count) + 1 AS deptrank_percentage
        , FLOOR((class_rank::DECIMAL - 1)*100::DECIMAL / class_count) + 1 AS classrank_percentage
        , FLOOR((tag1_rank::DECIMAL - 1)*100::DECIMAL / tag1_count) + 1 AS tag1rank_percentage
...
";
                ...
                StringBuilder sbSubjectSkillDomainScore = new StringBuilder();
                foreach (GradeYearSemesterInfo gs in calSemesterList)
                {
                    string ka = "";
                    if (gs.Semester == 1)
                    ...
                    if (inString != "")
                    {
                        string str = @", subject_score_SkillDomain_avg_" + ka + @" AS
(
    SELECT
        student_list.student_id
        ,student_list.student_name
    ...
    INNER JOIN student_list
        ON subject_score.student_id = student_list.student_id
    WHERE 
        subject_score.grade_year = " + gs.GradeYear + @" AND subject_score.semester = " + gs.Semester + @"  AND subject IN(" + inString + @") ---AND subject_entry = '學業'
    GROUP BY 
    student_list.student_id
        ,student_list.student_name
        ,student_list.rank_grade_year
...
";
                        sbSubjectSkillDomainScore.AppendLine(str);
...
SELECT
        ...
        ,sum_credit
        FROM 
        subject_score_SkillDomain_avg_1a
UNION All 
SELECT
        student_id
        ,student_name
        ...
        ,sum_credit
        FROM 
        subject_score_SkillDomain_avg_1b
UNION All 
SELECT
        student_id
        ,student_name
        ...
        ,sum_credit
        FROM 
        subject_score_SkillDomain_avg_2a
UNION All 
SELECT
        student_id
        ,student_name
        ...
        ,sum_credit
        FROM 
        subject_score_SkillDomain_avg_2b
UNION All 
SELECT
        student_id
        ,student_name
        ...
        ,sum_credit
        FROM 
        subject_score_SkillDomain_avg_3a
            ) as subject_score_SkillDomain_avg5
group BY
student_id
        ,student_name
        ,rank_grade_year
...
";
                ...
                scNameList.Add("subject_score_skilldomainReq_avg_expand");
                scNameList.Add("subject_score_SkillDomain_avg_expand");
...
";
                    ...
                    if (sbScoreItemNum > 0 && sbScoreItemNum <= 10)  ///8+2 新增技能領域/部必
...
SELECT * from score_list limit 1
                ...
                + sbSubjectSore.ToString() + Strsubject_score_pro_avg_list  // 不分部必之專業實習
                +sbSubjectSore_req.ToString() + Strsubject_score_pro_avgreq_list
                + sbSubjectSkillDomainReqScore.ToString() + Strsubject_score_SkillDomainReq_avg_list  //部必技能領域
                + sbSubjectSkillDomainScore.ToString()+ Strsubject_score_SkillDomain_avg_list //不分部必之技能領域
                + sbSubjectChineseScore.ToString() + Strsubject_score_chinese_avg_list  // 不分部必之國文
                + sbSubjectChineseReqScore.ToString() + Strsubject_score_ChineseReq_avg_list 
                + sbSubjectEnglishScore.ToString() + Strsubject_score_english_avg_list // 不分部必之英文
                + sbSubjectEnglishReqScore.ToString() + Strsubject_score_EnglishReq_avg_list 
                + sbSubjectMathScore.ToString() + Strsubject_score_math_avg_list // 不分部必之數學
